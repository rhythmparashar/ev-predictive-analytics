...
def _parse_hhmmss_to_seconds(x: object) -> float:
    """Convert HH:MM:SS -> seconds (float)."""
...
def standardize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "total_running_hours" in df.columns:
        ...
        df["total_running_hours_s"] = df[
            "total_running_hours"
        ].apply(_parse_hhmmss_to_seconds)

        df = df.drop(columns=["total_running_hours"])

    if "last_trip_hours" in df.columns:

        df["last_trip_hours_s"] = df[
            "last_trip_hours"
        ].apply(_parse_hhmmss_to_seconds)
...
def run_ingest_for_day(
    ...
    if not csvs:
        raise FileNotFoundError(
            f"No raw CSVs found under {raw_dir}/dt={dt}/"
        )
    ...
    for csv_path in csvs:
        ...
        if fault_path.exists():
            ...
            faults = load_fault_csv(fault_path)

            fault_count = len(faults)

            silver = add_fault_flags(
                silver,
                faults
            )
        ...
        manifest["vehicles_processed"].append(
            vehicle_id
        )
        ...
        manifest["per_vehicle"][vehicle_id] = {
            ...
            "silver_duplicate_timestamps":
                int(silver["timestamp"].duplicated().sum()),

            "num_trips":
                int(silver["trip_id"].nunique(dropna=True)),
