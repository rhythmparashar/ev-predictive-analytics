from __future__ import annotations

from dataclasses import dataclass
from pathlib import Path
import pandas as pd
import yaml


def load_yaml(path: Path) -> dict:
    with open(path, "r", encoding="utf-8") as f:
        return yaml.safe_load(f) or {}


@dataclass
class TripState:
    vehicle_id: str
    trip_seq: int
    open_trip_id: str | None
    open_trip_start_ts: pd.Timestamp | None


def _read_state(state_path: Path) -> dict[str, TripState]:
    if not state_path.exists():
        return {}

    st = pd.read_parquet(state_path)
    out: dict[str, TripState] = {}

    for _, r in st.iterrows():
        out[str(r["vehicle_id"])] = TripState(
            vehicle_id=str(r["vehicle_id"]),
            trip_seq=int(r.get("trip_seq", 0)),
            open_trip_id=None if pd.isna(r.get("open_trip_id")) else str(r.get("open_trip_id")),
            open_trip_start_ts=None
            if pd.isna(r.get("open_trip_start_ts"))
            else pd.Timestamp(r.get("open_trip_start_ts")),
        )

    return out


def _write_state(state: dict[str, TripState], state_path: Path) -> None:
    state_path.parent.mkdir(parents=True, exist_ok=True)

    rows = [
        {
            "vehicle_id": vid,
            "trip_seq": int(s.trip_seq),
            "open_trip_id": s.open_trip_id,
            "open_trip_start_ts": s.open_trip_start_ts,
        }
        for vid, s in state.items()
    ]

    tmp = state_path.with_suffix(state_path.suffix + ".tmp")
    if tmp.exists():
        tmp.unlink()
    pd.DataFrame(rows).to_parquet(tmp, index=False)
    tmp.replace(state_path)


def add_trip_id(
    df: pd.DataFrame,
    trip_cfg_path: Path,
    quality_flags_path: Path,
    state_path: Path,
) -> pd.DataFrame:
    """
    Current implementation:
    gap-based session segmentation on Silver (1Hz).

    START
    -----
    First real telemetry row after a closed state.

    STOP
    ----
    GAP_INSERTED continues for gap_break_s seconds.

    Properties:
    ✔ Cross-day sessions allowed
    ✔ Midnight does NOT split sessions
    ✔ Only real telemetry gaps split sessions
    ✔ Deterministic
    ✔ Avoids per-row Python loops
    """

    cfg = load_yaml(trip_cfg_path)
    qf_map = load_yaml(quality_flags_path)

    gap_break_s = int(cfg.get("gap_break_s", 300))
    min_trip_duration_s = int(cfg.get("min_trip_duration_s", 60))
    gap_bit = int(qf_map["GAP_INSERTED"])

    df = df.sort_values(["vehicle_id", "timestamp"]).copy()
    df["trip_id"] = pd.NA

    state = _read_state(state_path)

    for vid, g in df.groupby("vehicle_id", sort=False):
        g = g.copy()

        st = state.get(vid) or TripState(
            vehicle_id=vid,
            trip_seq=0,
            open_trip_id=None,
            open_trip_start_ts=None,
        )

        if "quality_flag" not in g.columns:
            raise ValueError("trip_segmentor requires 'quality_flag' column")

        qf = g["quality_flag"].fillna(0).astype("int64")
        gap_inserted = (qf & gap_bit) != 0
        present = ~gap_inserted

        # Contiguous present/gap runs
        run_id = present.ne(present.shift(fill_value=present.iloc[0])).cumsum()

        trip_ids = pd.Series(pd.NA, index=g.index, dtype="object")

        open_trip_id = st.open_trip_id
        open_trip_start = st.open_trip_start_ts
        seq = st.trip_seq

        for _, idx in g.groupby(run_id, sort=False).groups.items():
            idx = list(idx)
            run_is_present = bool(present.loc[idx[0]])
            run_len = len(idx)

            if run_is_present:
                if open_trip_id is None:
                    seq += 1
                    open_trip_id = f"{vid}_{seq:06d}"
                    open_trip_start = pd.Timestamp(g.loc[idx[0], "timestamp"])

                trip_ids.loc[idx] = open_trip_id

            else:
                # Short telemetry gap remains inside the open session.
                if open_trip_id is not None and run_len < gap_break_s:
                    trip_ids.loc[idx] = open_trip_id

                # Long gap: only first (gap_break_s - 1) seconds stay inside session.
                elif open_trip_id is not None and run_len >= gap_break_s:
                    keep_n = max(gap_break_s - 1, 0)
                    if keep_n > 0:
                        trip_ids.loc[idx[:keep_n]] = open_trip_id
                    open_trip_id = None
                    open_trip_start = None

                # Closed state + gap run: remain NA

        g["trip_id"] = trip_ids

        # Min duration enforcement in 1Hz rows
        sizes = g["trip_id"].value_counts(dropna=True)
        short = sizes[sizes < min_trip_duration_s].index
        if len(short) > 0:
            g.loc[g["trip_id"].isin(short), "trip_id"] = pd.NA

            # If the open trip got nulled for being too short, clear persisted state
            if open_trip_id in set(short):
                open_trip_id = None
                open_trip_start = None

        df.loc[g.index, "trip_id"] = g["trip_id"]

        state[vid] = TripState(
            vehicle_id=vid,
            trip_seq=seq,
            open_trip_id=open_trip_id,
            open_trip_start_ts=open_trip_start,
        )

    _write_state(state, state_path)
    return df